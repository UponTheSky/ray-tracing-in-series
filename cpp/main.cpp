...
int main() {
  ...
  color background(
      (double)(bgcolor_json["r"].i),
      (double)(bgcolor_json["g"].i),
      (double)(bgcolor_json["b"].i));
  int max_depth = config_json["max_depth"].i;

  // camera parameters
  JsonMap lookfrom_json = (*config_json["lookfrom"].json);
  point3 lookfrom(
      (double(lookfrom_json["x"].i)),
      (double(lookfrom_json["y"].i)),
      -(double(lookfrom_json["z"].i)));

  JsonMap lookat_json = (*config_json["lookat"].json);
  point3 lookat(
      (double(lookat_json["x"].i)),
      (double(lookat_json["y"].i)),
      (double(lookat_json["z"].i)));
